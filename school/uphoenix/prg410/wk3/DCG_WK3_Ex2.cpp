...
// September 29, 2007
//
// 2. Repeat the previous exercise but use pointer notation throughout instead
//    of arrays.
//
...
// forward reference
int* reallocArray(int* oldArray, int oldSize, int newSize);
...
int main()
{
  ...
  int* values = static_cast<int*>(malloc(sizeof(int) * maxSize));
 ...
 */
int* reallocArray(int* oldArray, const int oldSize, const int newSize)
{
  int* newArray = static_cast<int*>(malloc(sizeof(int) * newSize));
  if (newArray == NULL)
  {
