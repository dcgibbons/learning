...
public:  
  ...
  Mortgage(const double interestRate, const int term, const double principalAmount = 0.0);
  ~Mortgage();
  ...
  double getInterestPayment(const int month) const throw(std::invalid_argument);
  double getPrincipalPayment(const int month) const throw(std::invalid_argument);
  double getRemainingBalance(const int month) const throw(std::invalid_argument);
