/********************************************************************
 Program Name: gibbons_week3.cpp
...
int main()
{
  ...
  cout << "\tMortgage Calculator - Week 3" << endl
    ...
    m_monthlyPayment(calculateMonthlyPayment())
{
  ...
  for (int month = 0; month < m_numPayments; month++)
  {
    ...
    m_interestPayments[month] = interestRate;
...
const bool MortgageCalculator::getMortgageTerms()
{
  ...
  m_interestRate = MORTGAGE_CHOICES[mortgage - 1].getInterestRate();
  m_term = MORTGAGE_CHOICES[mortgage - 1].getTerm();
