/********************************************************************
 ...
 Program Description: Change Request # 15
   ...
   select from a menu of mortgage loans: 7 year at 5.35%, 15 year at 5.5%, 
   and 30 year at 5.75% (use an array for the mortgage information and read 
   the interest rates to fill the array from a sequential file). In either 
   case, display the mortgage payment amount. Then, list the loan balance 
   and interest paid for each payment over the term of the loan. On longer 
   term loans, the list will scroll off the screen. Do not allow the list to 
   scroll off the screen, but rather display a partial list and then allow 
   the user to continue the list. Allow the user to loop back and enter a new 
...
#include "gibbons_week5.h"

int main()
{
  // display a welcome banner
  cout << "\tMortgage Calculator - Week 5" << endl
 ...
 **********************************************************************/
Mortgage::Mortgage(const double interestRate, 
                   const int term, 
...
/********************************************************************
 ...
 Member Description: constant for the predefined mortgages available

 **********************************************************************/
const Mortgage* MortgageCalculator::MORTGAGE_CHOICES[MAX_MORTGAGES];

/********************************************************************
 Function Name: MortgageCalculator::MortgageCalculator

 Function Description: constructs a new mortgage calculator
...
/********************************************************************
 ...
 Function Description: cleanup of mortgage calculator data

 **********************************************************************/
MortgageCalculator::~MortgageCalculator()
{
    delete MORTGAGE_CHOICES[0];
    delete MORTGAGE_CHOICES[1];
    delete MORTGAGE_CHOICES[2];
}

/********************************************************************
 Function Name: MortgageCalculator::loadInterestRates

 Function Description: loads dynamic rates from interest rate file

 **********************************************************************/
void MortgageCalculator::loadInterestRates()
{
  ifstream inputFile;
  inputFile.open("gibbons_rates.txt", ios::in);
  if (!inputFile)
  {
  ...
  int rateNo = 0;
  string line;
  double rates[MAX_MORTGAGES];

  while (rateNo < MAX_MORTGAGES && !inputFile.eof())
  {
    ...
    if (iss.fail())
    {
      cerr << "Invalid interest rate value at line " << lineNo << endl;
    }
    else
    {
      rates[rateNo++] = rate;
  ...
  MORTGAGE_CHOICES[0] = new Mortgage(rates[0], 7);
  MORTGAGE_CHOICES[1] = new Mortgage(rates[1], 15);
  MORTGAGE_CHOICES[2] = new Mortgage(rates[2], 30);
}

/********************************************************************
 Function Name: MortgageCalculator::trim

 Function Description: trims whitespace from the start and and of a string 
 
 **********************************************************************/
void MortgageCalculator::trim(string& str)
{
