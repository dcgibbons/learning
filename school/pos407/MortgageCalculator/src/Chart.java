 ...
 *   2.00  | 2006-02-03 | Merged Week 4 and Week 5 assignments into an Applet
...
public class Chart extends JComponent {
    ...
    public void paintComponent(final Graphics g) {
        ...
        final double xStep = dataMaxX / (xEnd - xStart);
        final double yStep = dataMaxY / (yEnd - yStart);
        ...
        plotIntersects(xStep, xStart, xEnd, yStep, yStart, yEnd, g2d, fm);
     ...
     */
    private void plotIntersects(final double xStep,
                                final float xStart,
                                final float xEnd,
                                final double yStep,
                                final float yStart,
        ...
        // calculate the intersection point between the data sets
        // TODO: this algorithm only works for two data sets and is using the
        // knowledge of an interest value in data set 0 and a principal value
        // in data set 1 - what's the right way?
        for (int i = 0; i < data[0].length; i++) {
            if (data[0][i] < data[1][i]) {
                xValue = i;
                xIntersect = i / xStep + xStart;
                yIntersect = data[1][i] / yStep + yStart;
                break;
            }
        }
        ...
        yPos += fm.getHeight();
        for (int set = 0; set < data.length; set++) {
            g2d.drawString(dataNames[set] + " " + axisFormats[Y_AXIS].format(data[set][xValue]),
                           xPos, yPos);
            yPos += fontHeight;
        }
        ...
        // the following draws an axis line at the intersection point, but it
        // seems like it complicates the drawing rather than helps it
/*
        ...
        final Line2D ln = new Line2D.Double(xStart,
                                            pt.getY(),
                                            xEnd,
                                            pt.getY());
        g2d.draw(ln);
